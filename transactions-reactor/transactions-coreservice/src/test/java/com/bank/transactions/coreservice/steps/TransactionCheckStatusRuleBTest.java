...
public class TransactionCheckStatusRuleBTest {
	...
	public void a_transaction_that_is_not_stored_in_our_system_B() throws Throwable {
	...
	public void i_check_the_status_from_any_channel_B(String client, String atm) throws Throwable {
		ruleCLIENT = new TransactionCheckStatusRule().defineRules();
		ruleATM = new TransactionCheckStatusRule().defineRules();
		transactionForStatusRuleBase = transactionForStatusRuleBase.clonebuilder().withDate(LocalDateTime.now().minusDays(1)).withChannel(Channel.fromString(client)).build();
		transactionForStatusRuleCLIENT = transactionForStatusRuleBase.clonebuilder().build();
		transactionForStatusRuleATM = transactionForStatusRuleBase.clonebuilder().build();
		factMapCLIENT.put(new Fact<TransactionForStatusRule>("transaction",transactionForStatusRuleCLIENT));
	...
	public void the_transaction_date_is_before_today_B() {
		Assertions.assertTrue(transactionForStatusRuleCLIENT.getDate().toLocalDate().isBefore(LocalDate.now()));
		Assertions.assertTrue(transactionForStatusRuleATM.getDate().toLocalDate().isBefore(LocalDate.now()));
	}

	@Then("The system returns the status {string}")
	public void the_system_returns_the_status_B(String string) throws Throwable {
	...
	public void the_amount_substracting_the_fee_B() {
